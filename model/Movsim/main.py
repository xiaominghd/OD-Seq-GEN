...
def main(opt):
    ...
    if opt.task == 'attention':
        ...
        generator = ATGenerator(device=device, total_locations=TOTAL_LOCS, starting_sample='rand',
    ...
    if opt.pretrain:
        ...
        logger.info('begin load generator and discriminator ...')
    ...
    # discriminator.load_state_dict(torch.load( DATA_PATH+'/%s/pretrain/discriminator.pth' % opt.data))
    # print('')
    logger.info('advtrain generator and discriminator ...')
    ...
    for epoch in range(EPOCHS):
        ...
        logger.info("Current JSD: %f, %f, %f, %f, %f, %f" % (JSDs[0], JSDs[1], JSDs[2], JSDs[3], JSDs[4], JSDs[5]))
        ...
        for it in range(1):
            ...
            tim = torch.LongTensor([i % 12 for i in range(24)]).to(device)
